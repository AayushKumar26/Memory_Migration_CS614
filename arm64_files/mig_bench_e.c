/*
 * mig_bench_e.c — Benchmark E: Multi-Threaded NUMA Migration Characterization
 ...
 *
 * Outputs (per config):
 *   bench_e_workers_<label>.csv   — per-thread per-phase application stats
 *   bench_e_migrator_<label>.csv  — migration throughput/failure stats
 *   bench_e_timing_<label>.csv    — kernel ring buffer 5-stage timing sample
 *
...
/*
 * now_ns — monotonic nanosecond timestamp.
 * On ARM64, clock_gettime(CLOCK_MONOTONIC) is a VDSO call that reads
 * CNTVCT_EL0 directly without entering the kernel. Cost ≈ 5–10 ns.
 */
 ...
 * MAX_SAMPLES entries. Two clock_gettime calls bracket the target
 * memory operation; at VDSO speed (~5–10 ns each), the overhead is
...
{
    char path[256];
    snprintf(path, sizeof(path), "bench_e_workers_%s.csv", sh->prefix);
    FILE *f = fopen(path, "w");
...
{
    char path[256];
    snprintf(path, sizeof(path), "bench_e_migrator_%s.csv", sh->prefix);
    FILE *f = fopen(path, "w");
                      ...
                      int node0_cpu, int *n1cpus, int nn1)
{
    ...
    snprintf(rbuf_path, sizeof(rbuf_path),
             "bench_e_timing_%s.csv", sh->prefix);
...
int main(int argc, char *argv[])
{
    ...
    printf("║   ARM64 / Linux 6.1.4 / 512 MB buf / numa=fake=2    ║\n");
    printf("╚══════════════════════════════════════════════════════╝\n\n");
    ...
    for (int i = start; i < end; i++) {
        ...
        /* Clamp thread count to available node1 CPUs */
        if (c->nthreads > nn1) {
            fprintf(stderr,
                "  NOTE: clamping %d workers to %d (node1 CPU count)\n",
                c->nthreads, nn1);
            c->nthreads = nn1;
        }
    ...
    printf("  Outputs: bench_e_workers_*.csv\n");
    printf("           bench_e_migrator_*.csv\n");
    printf("           bench_e_timing_*.csv  (ring buffer samples)\n");
    printf("  Analyze: python3 analyze_bench_e.py\n");
    printf("══════════════════════════════════════════════════════\n\n");
    return 0;
}
