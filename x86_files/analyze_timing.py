...
Target: x86_64, Linux 6.1.4, 4KB pages, real NUMA (2 nodes)
...
and produces:
  ...
  3. Sharing degree scaling analysis (rmap walk + IPI shootdown cost vs mapcount)
...
x86_64 interpretation notes (embedded in output):
  - Unmap cost: driven by INVLPG + TLB shootdown IPI to all online CPUs, not
    ARM64's hardware-broadcast TLBI. Cost scales with CPU count × mapcount.
  - Copy cost: REP MOVSB / ERMS (Enhanced REP MOVSB, Ivy Bridge+).
  - Lock CV > 1.0: normal — x86_64 LOCK BTS on folio->flags can see
    cache-line bouncing from concurrent page reclaim or IRQ handlers.
"""
...
def analyze_stage_breakdown(data, label):
    ...
    print(f"  n={n}  |  x86_64 / 4KB pages / real NUMA (2 nodes)")
    print("=" * 84)
    ...
    # ── Key Insights ──
    print()
    print("  KEY INSIGHTS (x86_64-specific):")
    ...
    if "copy_ns" in stage_means and "unmap_ns" in stage_means:
        ...
        if c > u:
            ...
            print(f"  • Unmap/Copy = {u/c:.1f}×  → IPI shootdown reduction is highest priority")
    ...
    for col, name in zip(STAGES, STAGE_NAMES):
        ...
        if cv > 1.0:
            print(f"  • {name}: CV={cv:.2f} (high) — likely cache-line bouncing "
                  f"on x86_64 LOCK BTS on folio->flags or scheduler jitter")
        elif cv > 0.5:
            print(f"  • {name}: CV={cv:.2f} (moderate) — IPI round-trip timing variation")
    ...
    if "lock_contended" in data:
        ...
        if nc > 0:
            ...
            print(f"  • Lock contention: none — all uncontended "
                  f"(fast LOCK BTS path)")

    # Unmap x86_64 commentary
    if "unmap_ns" in data:
        ...
        print(f"    x86_64: cost = rmap walk + INVLPG + IPI shootdown × online CPUs per PTE")
...
def analyze_size_comparison(data_4kb, data_2mb):
    ...
    print("  x86_64 / 4KB base pages / real NUMA")
    print("=" * 70)
    ...
    if "copy_ns" in data_4kb and "copy_ns" in data_2mb:
        ...
        print(f"    x86_64 note: REP MOVSB (ERMS) benefits from hardware")
        print(f"    prefetcher activation on the longer 2MB copy path;")
        print()
        print(f"  OPTIMISATION PRIORITY:")
        print(f"  • For 4KB pages: O(1) stages dominate → reduce IPI shootdown count")
...
def analyze_sharing_scaling(files_by_degree):
    """
    Analyse how unmap (IPI shootdown per-mapping) and remap costs scale
    with sharing degree.

    Expected pattern on x86_64:
      unmap_ns ≈ α × degree + β   (linear: one INVLPG + IPI per PTE per page)
      remap_ns ≈ γ × degree       (linear: one PTE write per mapping)
      copy_ns  ≈ constant         (data size unchanged)

    α = per-PTE INVLPG + IPI round-trip cost (scales with online CPU count)
    ...
    print()
    print("=" * 80)
    print("  SHARING DEGREE SCALING (rmap walk + IPI shootdown)")
    print("  x86_64: one INVLPG + IPI to all online CPUs per PTE per sharing process")
    print("=" * 80)
    ...
    if len(rows) >= 2:
        ...
        print(f"    Unmap:  α={u_slope/1e3:6.2f} µs/process,  "
              f"β={u_int/1e3:6.2f} µs  (per-IPI round-trip + rmap walk cost)")
        print(f"    Remap:  α={r_slope/1e3:6.2f} µs/process,  "
              f"β={r_int/1e3:6.2f} µs  (per-PTE write cost)")
        print()
        print("  x86_64 IPI INTERPRETATION:")
        print(f"    Per-process IPI overhead ≈ {u_slope/1e3:.2f} µs")
        print(f"    This is INVLPG + TLB shootdown IPI per PTE mapping.")
        print(f"    Reducing shootdowns (e.g. lazy TLB, batched invalidation)")
...
def plot_sharing_scaling(rows):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(degrees, unmaps, "o-", color="#e74c3c", label="Unmap (INVLPG+IPI)")
...
def analyze_try_migrate(data, label):
    ...
    Breaks Stage 2 (unmap_ns) into two components:
      try_migrate_ns  — time inside try_to_migrate() itself
                        (rmap walk + per-PTE INVLPG + IPI shootdown to all CPUs)
    ...
    achievable from reducing IPI shootdown frequency.
    """
    ...
    print("  " + "─" * 65)
    print(f"  {'try_to_migrate() [rmap+INVLPG+IPI]':<35}  "
    ...
    print()
    print(f"  IPI SHOOTDOWN IMPLICATION:")
    print(f"  • try_to_migrate() is {frac_tm:.1f}% of Stage 2")
    print(f"  • This is the portion that scales with mapcount (O(N) IPI round-trips)")
    print(f"  • Fixed overhead ({oh_mean/1e3:.2f} µs) is irreducible regardless of strategy")
    print(f"  • Reducing shootdowns (batching, lazy TLB) saves the O(N) component")
...
def main():
    ...
    for f in files_2mb:
        ...
        if d is None:
            continue
        label = f"2MB THP ({os.path.basename(f)})"
        analyze_stage_breakdown(d, label)
        analyze_try_migrate(d, label)
        prefix = f.replace(".csv", "")
        plot_breakdown(d, label, prefix)
